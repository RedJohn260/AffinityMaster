...
:build_executable
echo Step 5: Building PlayGTAIV.exe...

if not exist PlayGTAIV.py (
    echo PlayGTAIV.py not found. Ensure the file exists in this directory.
    pause >nul
    exit /b
)
...
pyinstaller --clean --onefile PlayGTAIV.py
if exist dist\PlayGTAIV.exe (
...
:: === Step 6: Copy PlayGTAIV.ini to dist folder ===
echo Step 6: Copying PlayGTAIV.ini to dist folder...
if exist PlayGTAIV.ini (
    copy PlayGTAIV.ini dist\
    if %errorlevel% equ 0 (
        echo PlayGTAIV.ini copied successfully.
    ) else (
        echo Failed to copy PlayGTAIV.ini.
    )
) else (
    echo PlayGTAIV.ini not found. Skipping copy.
)
